...
# ggsave("5.figures/I.kraken2.db.comparison.domain.counts.pdf", p, width = 10,height = 6)
...
alpha.diversity.longer <- alpha.diversity %>% pivot_longer(c("Observed","Shannon", "Simpson"), names_to = "alpha_index", values_to = "values")
```
...
p <- ggplot(alpha.diversity.longer, aes(x=database, y=values))+
  ...
  # stat_pvalue_manual(alpha.stat.db, label = "p.adj.signif", hide.ns = TRUE,size=5,bracket.size = 0.3, tip.length = 0.01)+
...
p
getwd()
# ggsave("5.figures/I.kraken2.db.comparison.alpha.indices.pdf", p, width = 10, height=7)
...
pheatmap::pheatmap(beta_dist_db, col = colorRampPalette(c("red", "#FFA701","lightyellow"))(50), annotation = metadata, annotation_colors = annot_colors,legend = FALSE, annotation_legend = FALSE,silent = TRUE,  treeheight_row = 0, treeheight_col = 30, main = c(eval(phyloseq_obj)))
...
databases <- list("minikraken", "standard", "customized", "maxikraken")
...
# ggsave("5.figures/I.kraken2.db.comparison.beta.heatmap.pdf", combined_kraken2_heatmap, height=7, width = 10)
...
for (db in databases){
  ...
  colnames(beta_dist_db_df_combined) <-  c(eval(db), "comparison")
